...
PR: await (up[self] /\ (\A k \in UpNodes: pt[self]<=pt[k]));                 
    {   
        prev_value:=d[self];                    
        ...
        if(prev_mb_count # Cardinality(mb[self]) \/ prev_value # d[self]) ctr:=1;
        else ctr:=ctr-1;
        
        prev_mb_count:=Cardinality(mb[self]);  \*\* set the message count of current step to prev_mb_count
        mb[self]:={};                   \*\* clear the mailbox for next round
        
        if(ctr=0) t[self]:=TRUE;          \*\* when no more rounds are needed for the current node terminate the node
...
vars == << failNum, up, pt, t, d, mb, fail, pc, v, prev_value, Q, 
           prev_mb_count, ctr >>
...
P(self) == /\ pc[self] = "P"
           /\ IF up[self] /\ ctr[self]>0
                 ...
                 ELSE /\ pc' = [pc EXCEPT ![self] = "Done"]
                      /\ UNCHANGED << v, Q >>
           /\ UNCHANGED << failNum, up, pt, t, d, mb, fail, prev_value, 
                           prev_mb_count, ctr >>
...
PS(self) == /\ pc[self] = "PS"
            ...
            /\ UNCHANGED << t, d, fail, v, prev_value, prev_mb_count, ctr >>

PR(self) == /\ pc[self] = "PR"
            /\ (up[self] /\ (\A k \in UpNodes: pt[self]<=pt[k]))
            /\ prev_value' = [prev_value EXCEPT ![self] = d[self]]
            /\ d' = [d EXCEPT ![self] = SetMin(mb[self])]
            /\ IF prev_mb_count[self] # Cardinality(mb[self]) \/ prev_value'[self] # d'[self]
